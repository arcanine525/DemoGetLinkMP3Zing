...
function getSourceLink(options, quanlity) {
    return new Promise((resolve, reject) => {
        request(options, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                //var link = body;
                //console.log(body);
                data = JSON.parse(body)['data'][quanlity]['link'];
...
async function sysn(oriLink) {
    ...
    let link128 = await getSourceLink(optionDownload, '128');
    let link320 = await getSourceLink(optionDownload, '320');
    let lossless = await getSourceLink(optionDownload, 'lossless');
    ...
    let o ={
        url: await getSourceLink(optionDownload, '320'),
        headers: headers,
        gzip: 'true'
    }
    ...
    //console.log("LINK 128: " + link128);
    // console.log("LINK 320 " + link320);
    // console.log("LINK LOSSLESS: " + lossless);
    console.log(arrLink)
    console.log("GETTTIIING DIRECT LINK.....")
    let dl = await getDirectLink(o);
    console.log(dl);
    console.log("DONE")
    
}

sysn("http://mp3.zing.vn/bai-hat/Lac-Nhau-Co-Phai-Muon-Doi-ERIK-ST319/ZW78D0FZ.html");
